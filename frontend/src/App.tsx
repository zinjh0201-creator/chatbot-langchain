  ...
  async function send() {
    ...
    try {
      ...
      const assistantMsg: ChatMessage = {
        ...
        role: 'assistant',
        text: data.answer,
        meta: { mode: data.mode, similarity: data.similarity ?? null, sources: data.sources ?? [] },
      }
    ...
    <div className="page">
      <aside className="sidebar">
        <div className="sidebarTitle">
          <span className="sidebarIcon" aria-hidden>📄</span>
          문서 업로드
        </div>
        ...
        >
          ...
          <span className="dropZoneLimit">Limit {MAX_PDF_MB}MB per file - PDF</span>
        </div>
        <button
          type="button"
          className="browseBtn"
      ...
      <main className="main">
        <header className="topbar">
          <div className="titles">
            ...
            <p className="subtitle">
              PDF 문서를 업로드하고 관련된 질문을 해보세요! (관련 문서가 없을 경우 기본 LLM 성능으로 답변합니다)
            </p>
          </div>
          <button
            type="button"
            ...
            <div className="empty">
              <div className="emptyCard">
                ...
                <div className="emptyDesc">
                  유사도 0.7 이상 문서가 있으면 <b>[문서 참조 답변]</b>, 아니면 <b>[Gemini 추론 답변]</b>으로
                  시작합니다.
                </div>
                  ...
                  {m.role === 'assistant' && m.meta?.mode ? (
                    <div className="meta">
                      <span className={`badge ${m.meta.mode}`}>
                        {m.meta.mode === 'document' ? '문서 참조' : 'Gemini 추론'}
                      </span>
                      {typeof m.meta.similarity === 'number' ? (
                        <span className="sim">top sim: {m.meta.similarity.toFixed(3)}</span>
                      ) : null}
        ...
        <div className="composer">
          ...
          <button
            ...
            aria-label="전송"
          >
            <span className="sendArrow" aria-hidden>↑</span>
          </button>
        </div>
      </main>
    </div>
